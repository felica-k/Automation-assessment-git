...
public class ConfirmationTest extends BaseTest {
	...
	public void verifyItemTotalTest(String product1, String product2, String fName, String lName, String zip) {
		...
		double expectedItemTotal = Double.parseDouble(itemTotal);
		this.orderPage.doClickFinish();
		Assert.assertEquals(actualItemTotal, expectedItemTotal);
	}

	@Test(priority = 3)
	public void verifyOrderConfirmation() {
		String stmt = this.orderHistory.orderConfirmation();
		Assert.assertEquals(stmt, "Thank you for your order!");
	}
}
